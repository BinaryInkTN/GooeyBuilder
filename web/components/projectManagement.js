...
export function generateProjectXML() {
    ...
    windowElement.setAttribute("debug_overlay", state.previewWindow.dataset.debug_overlay || "false");
    windowElement.setAttribute("cont_redraw", state.previewWindow.dataset.cont_redraw || "false");
    windowElement.setAttribute("is_visible", state.previewWindow.dataset.is_visible || "true");
    windowElement.setAttribute("is_resizable", state.previewWindow.dataset.is_resizable || "true");
    ...
    function saveWidget(widget, parentElement) {
        ...
        switch (widget.dataset.type) {
            ...
            case "List":
                ...
                break;
            case "Progressbar":
                widgetElement.setAttribute("value", widget.dataset.value || "50");
                break;
            case "Meter":
                widgetElement.setAttribute("value", widget.dataset.value || "50");
                widgetElement.setAttribute("label", widget.dataset.label || "Meter");
                break;
            case "GSwitch":
                widgetElement.setAttribute("showHints", widget.dataset.showHints || "false");
                widgetElement.setAttribute("state", widget.classList.contains("checked") ? "true" : "false");
                break;
            case "Tabs":
                widgetElement.setAttribute("isSidebar", widget.dataset.isSidebar || "false");
                break;
            case "Plot":
                widgetElement.setAttribute("plotType", widget.dataset.plotType || "line");
                break;
        }

        if (widget.dataset.type !== "Slider" && widget.dataset.type !== "Image" && 
            widget.dataset.type !== "DropSurface" && widget.dataset.type !== "Canvas" &&
            widget.dataset.type !== "Plot" && widget.dataset.type !== "Progressbar" &&
...
export function saveProjectToXML() {
    try {
        ...
        document.getElementById("status-text").textContent = "Error saving project";
    }
}
        ...
        reader.onload = (event) => {
            try {
                ...
                if (windowElement) {
                    ...
                    state.previewWindow.dataset.debug_overlay = windowElement.getAttribute("debug_overlay") || "false";
                    state.previewWindow.dataset.cont_redraw = windowElement.getAttribute("cont_redraw") || "false";
                    state.previewWindow.dataset.is_visible = windowElement.getAttribute("is_visible") || "true";
                    state.previewWindow.dataset.is_resizable = windowElement.getAttribute("is_resizable") || "true";
                }
                    ...
                    function loadWidget(widgetElement, parent = null) {
                        ...
                        switch (type) {
                            ...
                            case "List":
                                ...
                                break;
                            case "Progressbar":
                                widget.dataset.value = widgetElement.getAttribute("value") || "50";
                                break;
                            case "Meter":
                                widget.dataset.value = widgetElement.getAttribute("value") || "50";
                                widget.dataset.label = widgetElement.getAttribute("label") || "Meter";
                                break;
                            case "GSwitch":
                                widget.dataset.showHints = widgetElement.getAttribute("showHints") || "false";
                                if (widgetElement.getAttribute("state") === "true") {
                                    widget.classList.add("checked");
                                }
                                break;
                            case "Tabs":
                                widget.dataset.isSidebar = widgetElement.getAttribute("isSidebar") || "false";
                                break;
                            case "Plot":
                                widget.dataset.plotType = widgetElement.getAttribute("plotType") || "line";
                                break;
                        }
...
export function setupEditors() {
    ...
    CodeMirror.registerHelper("hint", "xml", (cm) => {
        ...
        const token = cm.getTokenAt(cur);
        const tags = [
            "project", "window", "widgets", "widget",
            "callback", "children"
        ];
        const attributes = [
            "title", "width", "height", "x", "y",
            "type", "id", "minValue", "maxValue",
            "showHints", "relativePath", "dropsurfaceMessage",
            "dropdownOptions", "listOptions", "text",
            "name", "value", "label", "state", "isSidebar",
            "plotType", "version", "platform", "language",
            "debug_overlay", "cont_redraw", "is_visible",
            "is_resizable"
        ];
